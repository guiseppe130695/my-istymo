<?php
/**
 * Gestionnaire de configuration pour les cartes de succession
 * Intégration avec Gravity Forms
 */

if (!defined('ABSPATH')) exit;

class Carte_Succession_Config_Manager {
    private static $instance = null;
    private $config_key = 'my_istymo_carte_succession_config';
    ...
    public function register_settings() {
        register_setting(
            'my_istymo_carte_succession_settings',
            $this->config_key,
    ...
    public function get_form_fields($form_id) {
        if (!class_exists('GFAPI')) {
            error_log('Carte Succession Debug: GFAPI class not found');
            return array();
        }
        ...
        if (is_wp_error($form)) {
            error_log('Carte Succession Debug: Error getting form - ' . $form->get_error_message());
            return array();
        }
        
        if (!$form) {
            error_log('Carte Succession Debug: Form not found for ID ' . $form_id);
            return array();
        }
        ...
        error_log('Carte Succession Debug: Form structure - ' . print_r($form, true));
        
        // Essayer différentes approches pour récupérer les champs
        if (isset($form['fields']) && is_array($form['fields'])) {
            error_log('Carte Succession Debug: Found ' . count($form['fields']) . ' fields in form[fields]');
            foreach ($form['fields'] as $field) {
                if (is_object($field)) {
                    ...
                    if ($field_id) {
                        ...
                        error_log('Carte Succession Debug: Added field ID ' . $field_id . ' - ' . $fields[$field_id]['label']);
                    }
                }
            }
        } else {
            error_log('Carte Succession Debug: No fields found in form[fields] or not an array');
        }
        
        // Si aucun champ trouvé, essayer avec GFFormsModel
        if (empty($fields) && class_exists('GFFormsModel')) {
            error_log('Carte Succession Debug: Trying GFFormsModel approach');
            $form_model = GFFormsModel::get_form($form_id);
            if ($form_model && isset($form_model['fields'])) {
                error_log('Carte Succession Debug: Found ' . count($form_model['fields']) . ' fields via GFFormsModel');
                foreach ($form_model['fields'] as $field) {
                    if (is_object($field) && isset($field->id)) {
                        ...
                        error_log('Carte Succession Debug: Added field ID ' . $field->id . ' - ' . $fields[$field->id]['label']);
                    }
                }
            }
        }
        
        error_log('Carte Succession Debug: Returning ' . count($fields) . ' fields');
        return $fields;
    }
    ...
    public function get_form_entries_paginated($form_id, $page = 1, $per_page = 20, $user_id = null) {
        ...
        error_log("Carte Succession Config - User ID: " . $user_id);
        error_log("Carte Succession Config - Is admin: " . (current_user_can('administrator') ? 'YES' : 'NO'));
        ...
        if ($user_id && !current_user_can('administrator')) {
            ...
            error_log("Carte Succession Config - Filter applied for user: " . $user_id);
        } else {
            error_log("Carte Succession Config - No filter applied (admin or no user_id)");
        }
        
        error_log("Carte Succession Config - Search criteria: " . print_r($search_criteria, true));
    ...
    public function get_form_entries_count($form_id, $user_id = null) {
        ...
        error_log("Carte Succession Config Count - User ID: " . $user_id);
        error_log("Carte Succession Config Count - Is admin: " . (current_user_can('administrator') ? 'YES' : 'NO'));
        ...
        if ($user_id && !current_user_can('administrator')) {
            ...
            error_log("Carte Succession Config Count - Filter applied for user: " . $user_id);
        } else {
            error_log("Carte Succession Config Count - No filter applied (admin or no user_id)");
        }
...
function carte_succession_config_manager() {
    return Carte_Succession_Config_Manager::get_instance();
}
