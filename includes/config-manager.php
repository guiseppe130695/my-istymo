...
class SCI_Config_Manager {
    ...
    public function config_page() {
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <!-- ✅ NOUVEAU : Statut du token INPI -->
                <?php
                $inpi_token_manager = sci_inpi_token_manager();
                $current_inpi_token = $this->get_inpi_token();
                $inpi_token_status = $inpi_token_manager ? $inpi_token_manager->check_token_validity(false) : false;
                ?>
                <div class="card" style="max-width: 600px; margin-bottom: 20px;">
                    <h3>📊 Statut du token INPI</h3>
                    <?php if ($current_inpi_token): ?>
                        <p><strong>Token actuel :</strong> <code><?php echo esc_html(substr($current_inpi_token, 0, 20) . '...'); ?></code></p>
                        <p><strong>Statut :</strong> 
                            <?php if ($inpi_token_status): ?>
                                <span style="color: green;">✅ Valide</span>
                            <?php else: ?>
                                <span style="color: red;">❌ Expiré ou invalide</span>
                            <?php endif; ?>
                        </p>
                        <p><small>Le token INPI est géré automatiquement via la page <a href="<?php echo admin_url('admin.php?page=sci-inpi-credentials'); ?>">Identifiants INPI</a></small></p>
                    <?php else: ?>
                        <p><span style="color: orange;">⚠️ Aucun token INPI disponible</span></p>
                        <p><small>Configurez vos identifiants dans la page <a href="<?php echo admin_url('admin.php?page=sci-inpi-credentials'); ?>">Identifiants INPI</a></small></p>
                    <?php endif; ?>
                </div>
                ...
                <table class="form-table">
                    <tr>
                        <th scope="row">
                            <label for="inpi_token">Token INPI (lecture seule)</label>
                        </th>
                        <td>
                            <input type="password" 
                                   id="inpi_token" 
                                   name="<?php echo $this->config_option_name; ?>[inpi_token]" 
                                   value="<?php echo esc_attr($current_inpi_token ?? ''); ?>" 
                                   class="regular-text" 
                                   placeholder="Token généré automatiquement" 
                                   readonly />
                            <p class="description">
                                Token d'authentification pour l'API INPI (généré automatiquement)<br>
                                <a href="<?php echo admin_url('admin.php?page=sci-inpi-credentials'); ?>" class="button button-secondary">Gérer les identifiants INPI</a>
                            </p>
                        </td>
                    </tr>
        ...
        /* ✅ NOUVEAU : Styles pour la carte de statut */
        .card {
            background: #fff;
    ...
    public function is_configured() {
        // ✅ MODIFIÉ : Vérifier le token INPI depuis le gestionnaire de tokens
        $inpi_token = $this->get_inpi_token();
        $laposte_token = $this->get_laposte_token();
        
        return !empty($inpi_token) && !empty($laposte_token);
    }
    ...
    public function get_inpi_token() {
        // ✅ MODIFIÉ : Récupérer le token depuis le gestionnaire de tokens INPI
        $inpi_token_manager = sci_inpi_token_manager();
        if ($inpi_token_manager) {
